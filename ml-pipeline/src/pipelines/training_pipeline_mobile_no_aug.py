...
def load_mobile_json_data(actions, source_dir):
    ...
    if not sequences:
        raise RuntimeError("No sequences loaded. Check dataset path and action folders.")

    return np.array(sequences), np.array(labels)


def train_and_save(run_name, source_dir, model_path, encoder_path):
    print("\n" + "=" * 70)
    print(f"Training Run: {run_name}")
    print("=" * 70)

    actions = load_actions()
    X, y = load_mobile_json_data(actions, source_dir)
...
def main():
    ...
    train_and_save(
        run_name="Mobile JSON NPY Only (No Augmentation)",
        source_dir=MOBILE_JSON_NPY_DIR,
        model_path=MODEL_MOBILE_JSON_NO_AUG,
        encoder_path=ENCODER_MOBILE_JSON_NO_AUG,
    )

    print("\nDone. Produced mobile JSON no-augmentation artifacts.")
