...
MODEL_ARTIFACTS = {
    "baseline": {
        ...
        "output_prefix": "action_model_baseline",
        "labels_prefix": "labels_baseline",
    },
    "augmented": {
...
def convert_model(model_key: str, output_dir: Path, quantize: bool) -> None:
    ...
    print(f"[{model_key}] Input shape: {model.input_shape}")
    print(f"[{model_key}] Output shape: {model.output_shape}")
    ...
    tflite_path = output_dir / f"{artifact['output_prefix']}{model_suffix}.tflite"
    tflite_path.write_bytes(tflite_model)

    labels = load_labels_for_artifact(artifact)
    labels_path = output_dir / f"{artifact['labels_prefix']}.json"
...
def load_labels_for_artifact(artifact: dict) -> list[str]:
    ...
    return [str(value) for value in encoder.classes_.tolist()]
